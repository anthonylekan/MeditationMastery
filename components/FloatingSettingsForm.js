...
function FloatingSettingsForm(props) {
    ...
    const clear_pro_settings = props.clear_pro_settings;
    const start = props.start;

    const [local_license_key, set_local_license_key] = useState(settings.license_key);
                    ...
                    <Container>
                        ...
                        <Form>
                            ...
                            <Form.Field hidden={is_pro}>
                                ...
                                <Input value={local_license_key} icon={{ name: 'arrow alternate circle right outline', circular: true, link: true, onClick: (e) => { update_setting('license_key', local_license_key); } }} placeholder='Enter License Key' onChange={(e) => { set_local_license_key(e.target.value); }}/>
                            </Form.Field>

                            <Form.Field disabled={!is_pro}>
                                <label>Floating Text</label>
                                <Input value={settings.floating_text} placeholder='Display a message during your session' onChange={handle_text('floating_text')}/>
                            </Form.Field>
                            ...
                            { is_pro && <Button basic={true} negative={true} onClick={() => { clear_pro_settings(); }} >Sign Out</Button> }
                        </Form>
                    </Container>
                </div>
